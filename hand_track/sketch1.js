...
setup = () => {
    ...
    noStroke();
    btn1 = new HButton((width / 2) - 200, height - 200, "Station");
    btn2 = new HButton((width / 2), height - 200, "Line");
    btn3 = new HButton((width / 2) + 200, height - 200, "Nearby");
}
...
draw = () => {
    ...
    if (controller == "Station") {
        text("Hi! Welcome to Mak Island Station. There are three exits on this site. Maidao Station is the middle station of Qingdao Metro Line 2. Opened on December 10, 2017.", width / 2, height - 450);
    }
    if (controller == "Line") {
        text("Qingdao Metro Line 2 starts at Taishan Road Station, passes through the four districts and ends at Licun Park Station. The symbol color is red.", width / 2, height - 450);
    }
    if (controller == "Nearby") {
        text("After exiting from Exit B of this station, walk about 436 meters to Qingdao University Fushan Campus (East Gate).", width / 2, height - 450);
